...
const SHIFTS = [
  ...
  },
  {
    ...
    bg: '#EFF6FF', border: '#BAE6FD', text: '#075985', accent: '#0EA5E9', iconBg: '#E0F2FE',
  },
  {
    ...
    bg: '#F0FDF4', border: '#BBF7D0', text: '#166534', accent: '#22C55E', iconBg: '#DCFCE7',
  },
];
      ...
      <div className="w-full max-w-[430px] min-h-[100dvh] bg-slate-50 flex flex-col shadow-[0_0_0_1px_rgba(0,0,0,.04),0_8px_48px_rgba(0,0,0,.08)] relative overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-[14px] pt-[14px] pb-[110px] flex flex-col gap-2.5">
          ...
          <div className="flex items-center justify-between py-1">
            <div className="text-[14px] font-extrabold text-cyan-800 tracking-[-0.2px]">Absen Karyawan Valet</div>
          ...
          <div className="bg-white rounded-[18px] px-4 py-[14px] flex items-start gap-3 shadow-[0_1px_3px_rgba(0,0,0,.04),0_0_0_1px_rgba(0,0,0,.03)] animate-fade-up">
            ...
            <div>
              ...
              <div className="text-[11px] text-slate-400 leading-[1.55] font-medium">
                Aksi valet hanya bisa dilakukan dalam radius {MAX_DIST_M}m dari lokasi kantor. Pastikan GPS aktif dan sinyal stabil.
              </div>
            </div>
          </div>
          ...
          <div className="pointer-events-auto w-full max-w-[430px] bg-white/92 backdrop-blur-[20px] border-t border-slate-200/60 px-5 pt-1.5 pb-safe-6 shadow-[0_-4px_24px_rgba(0,0,0,.06)]">
            <nav className="grid grid-cols-3 gap-1">
              {[
                ...
                return (
                  <Link key={to} to={to}
                    className={`relative flex flex-col items-center gap-1 px-2 py-2 pb-1.5 rounded-[14px] no-underline text-[10px] font-semibold tracking-[.02em] transition ${active ? 'text-cyan-700' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100'}`}>
                    {active && <span className="absolute top-0 left-1/2 -translate-x-1/2 w-5 h-[3px] rounded-b-[3px] bg-cyan-700"/>}
                    <Icon />
                    {label}
                  </Link>
                );
